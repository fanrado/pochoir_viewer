...
def test_shape_is_returned_and_matches_the_volume():
    volume, shape = volume_float32(ramp((4, 5, 6)))
...
def test_stride_thins_only_the_z_axis():
    volume, shape = volume_float32(ramp((4, 5, 6)), zstride=2)
...
def test_stride_keeps_the_first_layer():
    ...
    volume, _ = volume_float32(arr, zstride=3)
...
def test_stride_larger_than_the_axis_keeps_one_layer():
    volume, shape = volume_float32(ramp((3, 3, 4)), zstride=99)
...
def test_stride_of_the_dataset_shape():
    ...
    volume, shape = volume_float32(np.zeros((44, 44, 1601)), zstride=4)
...
def test_bytes_round_trip_as_a_float32_array():
    ...
    volume, shape = volume_float32(arr)
...
def test_bytes_are_in_c_order_x_major():
    ...
    arr = ramp((2, 3, 4))
    volume, (nx, ny, nz) = volume_float32(arr)
