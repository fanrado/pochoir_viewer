...
test("a z plane spans x by y and faces +z unrotated", () => {
  ...
  assert.ok(close(params.width, 0.3)); // (4 - 1) * 0.1
  assert.ok(close(params.height, 0.4)); // (5 - 1) * 0.1
  assert.deepEqual(params.rotation, [0, 0, 0]);
});

test("an x plane spans z by y and is rotated about y", () => {
  const params = slicePlaneParams("x", 0, meta());

  assert.ok(close(params.width, 0.5)); // (6 - 1) * 0.1
  assert.ok(close(params.height, 0.4));
  assert.deepEqual(params.rotation, [0, Math.PI / 2, 0]);
});

test("a y plane spans x by z and is rotated about x", () => {
  const params = slicePlaneParams("y", 0, meta());

  assert.ok(close(params.width, 0.3));
  assert.ok(close(params.height, 0.5));
...
test("the centre sits at the volume midpoint on the other two axes", () => {
  ...
  assert.ok(close(params.center[0], 0.15)); // spanX / 2
  assert.ok(close(params.center[1], 0.2)); // spanY / 2
});
